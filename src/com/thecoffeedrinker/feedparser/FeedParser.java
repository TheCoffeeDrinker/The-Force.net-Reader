...
public class FeedParser extends Parser{
	...
	private class ParserTask implements Runnable{
		...
		private void readFeed(XmlPullParser parser, String[] tag) throws XmlPullParserException, IOException {
			/**
			 * 1) Salta tutta l'intestazione finch� non arrivi al primo tag item
			 * 2) A questo punto, finch� non � stato trovato il tag item di chiusura analizza tutti i tag per 
			 * mettere via via in un oggetto item;
			 * 3) Continua a ciclare finch� non si arriva alla chiudura del tag channel.
			 */
			...
			while(parser.next()!=XmlPullParser.END_DOCUMENT){//finch� non termina il documento
